...
public class BoxInCenterOfScreenAndAverageColor extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("Box value: B", color.val[0]);
            telemetry.addData("Box value: G", color.val[1]);
            telemetry.addData("Box value: R", color.val[2]);
...
public class BoxInCenterOfScreenAndAverageColor extends LinearOpMode{
    ...
    public void runOpMode() {
        ...
        OpenCvPipeline pipeline = new OpenCvPipeline() {
            Scalar avgColor = new Scalar(0, 0, 0); // BGR

            @Override
            public Mat processFrame(Mat input) {
                Mat regionMat = input.submat(centerbox);
                avgColor = Core.mean(regionMat);
                Imgproc.rectangle(input, centerbox, new Scalar(255, 0, 0), 2);
                regionMat.release();
                return input;
            }
        ...
        while (opModeIsActive()) {
            ...
            Scalar color = ((OpenCvPipeline) pipeline).getAvgColor();
            telemetry.addData("Box value: B", color.val[0]);
            telemetry.addData("Box value: G", color.val[1]);
            telemetry.addData("Box value: R", color.val[2]);
            String detectedColor = color.val[2] > color.val[0] ? "Red" : "Blue";
    ...
    private class AverageColorPipeline {
    }
}
